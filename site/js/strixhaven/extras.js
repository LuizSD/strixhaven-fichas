...
export function renderExtra(m, p) {
  ...
  const classesAtuais = (p.classes?.length ? p.classes : [{ classe: p.classe }]).map(c => c.classe);
  const fora = m.classes?.length && !m.classes.some(c => classesAtuais.includes(c));
  return `<article class="magia-item sh-extra" data-extra-id="${escHtml(m.id)}"><h4>${rotuloLocalizado(m)} <span class="sh-selo">Extra</span></h4><p>${m.circulo ? `${escHtml(m.circulo)}º círculo` : 'Truque'} · ${escHtml(m.estado_extra)} · ${escHtml(cota)}</p>
    ...
    ${fora ? `<p class="spell-warning ${m.motivo ? 'info' : 'error'}">⚠ Fora da lista da sua classe (${escHtml(classesAtuais.join(' / '))}). Listas normais: ${escHtml(m.classes.join(', '))}.${m.motivo ? ' Justificativa: '+escHtml(m.motivo) : ' Adição permitida; justificativa não informada.'}</p>` : ''}
